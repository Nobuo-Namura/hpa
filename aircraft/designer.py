...
class HPADesigner():
    def __init__(self, n_div=4, max_plys = 10, level=0, AIRFOIL=False, WIRE=True, DIHEDRAL=False, PAYLOAD=False, FINE_MODE=False):
        ...
        files = np.array(['DAE11.xlsx', 'DAE21.xlsx', 'DAE31.xlsx', 'DAE41.xlsx']) # use the above line if your own airfoils are used
        ...
        self.re_max, self.re_min, self.aoa_max, self.aoa_min = re.max(), re.min(), aoa.max(), aoa.min()
        
        # parameters
        self.n_aero = max(200, self.n_div*10) if self.FINE_MODE else max(50, self.n_div*5)
        self.n_struc = 6*self.n_aero + 1
        self.gravity = 9.8
        self.rho = 0.1174*self.gravity #30 degree Celsius for worst flight condition with low air density
        ...
        self.lbound[self.n_div*2+1:self.n_div*2+2] = 4.0                            #angle of attack at wing root [deg]
        self.ubound[self.n_div*2+1:self.n_div*2+2] = 7.0                            #angle of attack at wing root [deg]
        ...
        if self.level==0:
            self.lbound[self.n_div*3+2:self.n_div*3+4] = 0.01                       #reduction factor of allowable strain for deflection constraint handling
            self.ubound[self.n_div*3+2:self.n_div*3+4] = 1.0                        #reduction factor of allowable strain for deflection constraint handling
        if self.level>0:
            self.lbound[self.n_div*3+2:self.n_div*4+2] = 0.8                        #beam diameter over max diameter of airfoil
            self.ubound[self.n_div*3+2:self.n_div*4+2] = 1.0                        #beam diameter over max diameter of airfoil
        if self.level==1:
            self.lbound[self.n_div*4+2:self.n_div*6+2] = 0.01                       #reduction factor of allowable strain for deflection constraint handling
            self.ubound[self.n_div*4+2:self.n_div*6+2] = 1.0                        #reduction factor of allowable strain for deflection constraint handling
        elif self.level==2:
            ...
            self.lbound[self.n_div*(4+m)+2:self.n_div*(4+m*2)+2] = self.delta_width #delta width of UD ply [m]
            self.ubound[self.n_div*(4+m)+2:self.n_div*(4+m*2)+2] = 260e-3           #delta width of UD ply [m] =pi*max radius=pi*1.5*0.11/2=0.0825pi=0.259
            self.lbound[self.n_div*(4+m*2)+2:self.n_div*(5+m*2)+2] = 0.0            #fixed position
            self.ubound[self.n_div*(4+m*2)+2:self.n_div*(5+m*2)+2] = 1.0            #fixed position
        if self.WIRE:
        ...
        if self.DIHEDRAL:
            ...
            self.ubound[self.i_x_dihedral] = self.max_dihedral_angle_at_root        #dihedral angle [deg]
        if self.PAYLOAD:
    ...
    def _x2param(self, x, NORMALIZED=False):
        ...
        ply_wing = []
        if self.level==0:
            y_diameter = np.array([(np.linspace(y_chord[i-1], y_chord[i], 100)*self.t_interp(np.linspace(y_airfoil[i-1], y_airfoil[i], 100))).min() for i in range(1, len(y_div))])
            y_diameter = np.hstack([y_diameter[0], y_diameter])
            phis = 30 + np.rad2deg(np.tile(self.delta_width*np.arange(self.max_plys), [self.n_div, 1])/(0.5*np.tile(y_diameter[1:], [self.max_plys,1]).T))
            phis = np.clip(phis, 0, 179)
            for i in range(self.n_div):
                if i == int(self.n_div/2):
                    plys = np.array([[90, 180, 0, 1, self.t_ply], [30, 180, 0, 1, self.t_ply], [-30, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply]])
                else:
                    plys = np.array([[90, 180, 0, 1, self.t_ply], [45, 180, 0, 1, self.t_ply], [-45, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply]])
            ...
            y_diameter = x[self.n_div*3+2:self.n_div*4+2]*np.array([(np.linspace(y_chord[i-1], y_chord[i], 100)*self.t_interp(np.linspace(y_airfoil[i-1], y_airfoil[i], 100))).min() for i in range(1, len(y_div))])
            y_diameter = np.hstack([y_diameter[0], y_diameter])
            if self.level==1:
                phis = 30 + np.rad2deg(np.tile(self.delta_width*np.arange(self.max_plys), [self.n_div, 1])/(0.5*np.tile(y_diameter[1:], [self.max_plys,1]).T))
                phis = np.clip(phis, 0, 179)
                ...
                length = x[self.n_div*4+2:self.n_div*(4+self.max_plys)+2].reshape(self.n_div, self.max_plys)
                length = 1 - np.clip(length.cumsum(axis=1), 0, 1)
                fixed = x[self.n_div*(4+self.max_plys*2)+2:self.n_div*(5+self.max_plys*2)+2].reshape(self.n_div, 1)
                fixed = sigmoid(fixed)
                tip = fixed + length*(1 - fixed)
                root = fixed*(1 - length)
                phis = x[self.n_div*(4+self.max_plys)+2:self.n_div*(4+self.max_plys*2)+2].reshape(self.n_div, self.max_plys)
                phis[:,0] -= self.delta_width
                phis = 30 + np.rad2deg(phis/(0.5*np.tile(y_diameter[1:], [self.max_plys,1]).T)).cumsum(axis=1)
                phis = np.clip(phis, 0, 179)
                for i in range(self.n_div):
                    if i == int(self.n_div/2):
                        plys = np.array([[90, 180, 0, 1, self.t_ply], [30, 180, 0, 1, self.t_ply], [-30, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply]])
                        ...
                        plys = np.array([[90, 180, 0, 1, self.t_ply], [45, 180, 0, 1, self.t_ply], [-45, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply]])
                    for j in range(self.max_plys):
                        plys = np.vstack([plys, [0, phis[i,j], root[i,j], tip[i,j], self.t_ply]])
                    ply_wing.append(plys)

        return span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire, wire_tension, dihedral_angle_at_root, payload, ply_wing, strain_ratios
    

    def _param2x(self, span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire=0, wire_tension=0, dihedral_angle_at_root=4.0, payload=0, ply_wing=[], strain_ratios=[]):
        ...
        if self.level==0:
            ...
            x[self.n_div*3+2:self.n_div*4+2] = y_diameter[1:]/np.array([(np.linspace(y_chord[i-1], y_chord[i], 100)*self.t_interp(np.linspace(y_airfoil[i-1], y_airfoil[i], 100))).min() for i in range(1, len(y_div))])
            if self.level==1:
                if len(strain_ratios) > 0:
                ...
                for i, plys in enumerate(ply_wing):
                    ...
                    length = 1 - length
                    x[self.n_div*4+2+self.max_plys*i:self.n_div*4+2+self.max_plys*i+len(length)] = np.hstack([length[0], np.diff(length)])
                    phis = np.deg2rad(np.diff(np.hstack([30, plys[n_entire:,1]])))*0.5*y_diameter[i+1]
                    phis[0] += self.delta_width
                    x[self.n_div*(4+self.max_plys)+2+self.max_plys*i:self.n_div*(4+self.max_plys)+2+self.max_plys*i+len(length)] = phis
                    x[self.n_div*4+2+self.max_plys*i+len(length):self.n_div*4+2+self.max_plys*(i+1)] = 1.0
    ...
    def baseline(self):
        ...
        plys1 = np.array([[90, 180, 0, 1, self.t_ply], [45, 180, 0, 1, self.t_ply], [-45, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply], [0, 30, 0, 1, self.t_ply], [0, 35, 0, 1.0, self.t_ply], [0, 40, 0, 1.0, self.t_ply], [0, 45, 0, 1.0, self.t_ply], [0, 50, 0, 1.0, self.t_ply], [0, 55, 0, 3.6/4.6, self.t_ply], [0, 65, 0, 2.6/4.6, self.t_ply], [0, 75, 0, 1.5/4.6, self.t_ply]])
        plys2 = np.array([[90, 180, 0, 1, self.t_ply], [45, 180, 0, 1, self.t_ply], [-45, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply], [0, 35, 0, 1, self.t_ply], [0, 50, 0, 1.0, self.t_ply], [0, 60, 0, 1.0, self.t_ply], [0, 65, 0, 1.0, self.t_ply], [0, 70, 0, 1.0, self.t_ply], [0, 75, 0, 1.0, self.t_ply], [0, 80, 0.5/4, 1, self.t_ply], [0, 85, 2.3/4, 1, self.t_ply]])
        plys3 = np.array([[90, 180, 0, 1, self.t_ply], [30, 180, 0, 1, self.t_ply], [-30, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply], [0, 30, 0, 1, self.t_ply], [0, 45, 0, 3.3/4.2, self.t_ply], [0, 55, 0, 2.9/4.2, self.t_ply], [0, 70, 0, 2.3/4.2, self.t_ply], [0, 80, 0, 1.8/4.2, self.t_ply], [0, 85, 0, 1.3/4.2, self.t_ply], [0, 90, 0, 0.85/4.2, self.t_ply], [0, 95, 0, 0.4/4.2, self.t_ply]])
        plys4 = np.array([[90, 180, 0, 1, self.t_ply], [45, 180, 0, 1, self.t_ply], [-45, 180, 0, 1, self.t_ply], [90, 180, 0, 1, self.t_ply], [0, 35, 0, 1, self.t_ply], [0, 45, 0, 2.15/3.2, self.t_ply], [0, 55, 0, 1.8/3.2, self.t_ply], [0, 65, 0, 1.4/3.2, self.t_ply], [0, 75, 0, 0.95/3.2, self.t_ply], [0, 85, 0, 0.45/3.2, self.t_ply]])
        ply_wing = [plys1, plys2, plys3, plys4]
        if self.level < 2:
            strain_ratios = np.array([[0.88, 0.88, 0.88, 0.88], [0.66, 0.66, 0.66, 0.66]])
        else:
            strain_ratios = np.ones([2, self.n_div])
        return self._param2x(span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire, wire_tension, dihedral_angle_at_root, payload, ply_wing, strain_ratios)
    ...
    def evaluate_performance(self, span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire, wire_tension, dihedral_angle_at_root, payload, n_plys_partial=[], strain_ratios=[], ply_wing=[]):
        if len(y_airfoil) == self.n_div:
            y_airfoil = np.hstack([y_airfoil, y_airfoil[-1]])
        if len(y_diameter) == self.n_div:
            y_diameter = np.hstack([y_diameter[0], y_diameter])
        self.x = self._param2x(span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire, wire_tension, dihedral_angle_at_root, payload, ply_wing, strain_ratios)
        self.span, self.y_div, self.y_chord, self.y_aoa, self.y_diameter, self.y_airfoil, self.y_wire, self.wire_tension, self.dihedral_angle_at_root, self.payload = span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire, wire_tension, dihedral_angle_at_root, payload
        ...
        if len(ply_wing) > 0:
            ...
            phis = 30 + np.rad2deg(np.tile(self.delta_width*np.arange(self.max_plys), [self.n_div, 1])/(0.5*np.tile(self.y_diameter[1:], [self.max_plys,1]).T))
            phis = np.clip(phis, 0, 179)
            self.ply_wing = []
            for i in range(self.n_div):
    ...
    def evaluate_performance_from_x(self, x, NORMALIZED=False):
        self.span, self.y_div, self.y_chord, self.y_aoa, self.y_diameter, self.y_airfoil, self.y_wire, self.wire_tension, self.dihedral_angle_at_root, self.payload, self.ply_wing, self.strain_ratios = self._x2param(x, NORMALIZED)
        if NORMALIZED:
    ...
    def _interpolate(self):
        section = pd.DataFrame(np.vstack([np.arccos(self.y_div), self.y_chord, self.y_aoa, self.y_diameter, self.y_airfoil]).T, columns=['theta', 'chord', 'aoa', 'diameter', 'airfoil'], index=self.y_div)
        section['source'] = 0
        theta = 0.5*np.pi*np.linspace(0, 1, self.n_aero+1)
        aero = pd.DataFrame(np.full([len(theta), 6], np.nan), columns=['theta', 'chord', 'aoa', 'diameter', 'airfoil', 'source'], index=np.cos(theta))
        aero['theta'] = theta
        aero['source'] = 1
    ...
    def _lifting_line(self):
        ...
        self.wing_area = np.sum((self.y_chord[:-1] + self.y_chord[1:])*np.diff(self.y_div)*0.5)*self.span
        ...
        induced_aoa = np.sum(ms*np.tile(xa, [self.n_aero,1])*np.sin(ms*thetas)/np.sin(thetas), axis=1)
    ...
    def _refine_grid(self, aero):
        ...
        fine_grid = pd.DataFrame(np.full([len(y), len(aero.columns)], np.nan), columns=aero.columns, index=y)
        fine_grid['source'] = 1
        aero['source'] = 0
    ...
    def _stiffness(self):
        ...
        for i, plys in enumerate(self.ply_wing):
            ...
            for j, ply in enumerate(plys_entire):
                ...
                phi = np.deg2rad(ply[1])
                mask = ply[4]*np.where((self.wing.index>=ply[2]*(y0_end - y0_start)+y0_start)&(self.wing.index<ply[3]*(y0_end - y0_start)+y0_start), 1, 0)
                sum_ply += mask
                Exx += mask*(self.E_T*nui*np.sin(axis)**4 + 2*ETL*(np.sin(axis)*np.cos(axis))**2 + self.E_L*nui*np.cos(axis)**4)
                Eyy += mask*(self.E_L*nui*np.sin(axis)**4 + 2*ETL*(np.sin(axis)*np.cos(axis))**2 + self.E_T*nui*np.cos(axis)**4)
                Exy += mask*(self.E_T*self.nu_L*nui*(np.sin(axis)**4 + np.cos(axis)**4) + ((self.E_L + self.E_T)*nui - 4*self.G_LT)*(np.sin(axis)*np.cos(axis))**2)
                Gxy += mask*(0.25*(self.E_L + self.E_T*(1 - 2*self.nu_L))*nui*np.sin(2*axis)**2 + self.G_LT*np.cos(2*axis)**2)
            ...
            R = np.where((self.wing.index>=y0_start)&(self.wing.index<y0_end), 0.5*self.diameter, 0)
            ...
            for j, ply in enumerate(plys_partial):
                ...
                if axis == 0:
                    ...
                    Exx = self.E_T*nui*np.sin(axis)**4 + 2*ETL*(np.sin(axis)*np.cos(axis))**2 + self.E_L*nui*np.cos(axis)**4
                    Eyy = self.E_L*nui*np.sin(axis)**4 + 2*ETL*(np.sin(axis)*np.cos(axis))**2 + self.E_T*nui*np.cos(axis)**4
                    Exy = self.E_T*self.nu_L*nui*(np.sin(axis)**4 + np.cos(axis)**4) + ((self.E_L + self.E_T)*nui - 4*self.G_LT)*(np.sin(axis)*np.cos(axis))**2
                    Ex = Exx - Exy**2/Eyy
                R = np.where((self.wing.index>=ply[2]*(y0_end - y0_start)+y0_start)&(self.wing.index<ply[3]*(y0_end - y0_start)+y0_start), 0.5*self.diameter, 0)
    ...
    def _evaluate_zerolift_deflection(self):
        ...
        self.deflection_angle_zerolift = np.hstack([0.0, 0.5*(self.M_EI_zerolift[1:] + self.M_EI_zerolift[:-1])*np.diff(self.wing.index.values)*self.span*0.5]).cumsum()
        self.deflection_zerolift = np.hstack([0.0, 0.5*(self.deflection_angle_zerolift[1:] + self.deflection_angle_zerolift[:-1])*np.diff(self.wing.index.values)*self.span*0.5]).cumsum()
    ...
    def _evaluate_flight_deflection(self):
        ...
        if self.wire_tension > 0:
            for i in range(iter_max):
                # wire
                wire_angle = np.arctan((self.z_wire + z_wire_deflection)/(self.y_wire*self.span*0.5)) if self.y_wire>0 else np.pi*0.5
                dihedral_angle_at_wire =  np.arctan(z_wire_deflection/(self.y_wire*self.span*0.5))
                axial_force = self.wire_tension*np.cos(wire_angle)/np.cos(dihedral_angle_at_wire)
                wire_load = axial_force*np.sin(wire_angle - dihedral_angle_at_wire)/np.cos(wire_angle)
                ...
                # moment & deflection
                self.total_load = self.local_lift -  self.wire_load - self.wing_weight*self.gravity
                self.moment, _ = self._compute_moment_with_axial_force(y=self.y0*self.span*0.5, F=self.total_load, T=self.axial_force, EI=self.EI)
                self.M_EI = self.moment/self.EI
                self.deflection_angle = np.hstack([0.0, 0.5*(self.M_EI[1:] + self.M_EI[:-1])*np.diff(self.y0)*self.span*0.5]).cumsum()
                self.deflection = np.hstack([0.0, 0.5*(self.deflection_angle[1:] + self.deflection_angle[:-1])*np.diff(self.y0)*self.span*0.5]).cumsum()
    ...
    def _evaluate_twist(self):
        ...
        self.torque = self.local_lift[:-1]*0.5*(self.chord[1:] + self.chord[:-1])*delta_center_of_pressure
    ...
    def _optimize_ply(self):
        def update_ply(j, y0_j, strain_const, y0_start, y0_end, root_limit, tip_limit):
            ...
            strain_zerolift = np.abs(self.moment_zerolift/(self.EI - self.EI_ply[:,j-4])*0.5*self.diameter)
            strain = np.max([strain/self.strain_ratios[0,i], strain_zerolift/self.strain_ratios[1,i]], axis=0)
        ...
        for i in range(len(self.ply_wing)-1, -1, -1):
            ...
            if max(strain1, strain2) > strain_max:
                ...
                for j in range(1, 180-int(phis.max()), 1):
                    ...
                    strain1 = (np.abs(M1/(self.EI[iy1] + np.sum(Ex*dIy1))*0.5*self.diameter[iy1]) + AF1/(self.EA[iy1] + np.sum(Ex*A1)))/self.strain_ratios[0,i]
                    strain2 = np.abs(M2/(self.EI[iy2] + np.sum(Ex*dIy2))*0.5*self.diameter[iy2])/self.strain_ratios[1,i]
                    if max(strain1, strain2) <= strain_max:
                        break
        ...
        for i in range(len(self.ply_wing)-1, -1, -1):
            ...
            for j in range(plys.shape[0]-1, 3, -1):
                ply = plys[j]
                y0_j = (self.y0>=ply[2]*(y0_end - y0_start)+y0_start)&(self.y0<ply[3]*(y0_end - y0_start)+y0_start)
                root, tip, root_limit, tip_limit = update_ply(j, y0_j, strain_max, y0_start, y0_end, root_limit, tip_limit)
            ...
            if sum_root*sum_tip > 0:
                ...
                if sum_root <= sum_tip:
                    for j in range(self.ply_wing[i].shape[0]-1, 3, -1):
                        if (self.ply_wing[i][j,3]>self.ply_wing[i][j,2]) and (self.ply_wing[i][j,2]>0):
                    ...
                    for j in range(self.ply_wing[i].shape[0]-1, 3, -1):
                        if (self.ply_wing[i][j,3]>self.ply_wing[i][j,2]) and (self.ply_wing[i][j,3]<1):
    ...
    def _compute_constraints(self):
        ...
        self.allowable_deflection = 0.5*self.span*np.sin(np.deg2rad(self.max_dihedral_angle_at_tip))
    ...
    def plot_wing_characteristics(self, marker='None', path='./', SAVE=False, suffix=''):
        # for visualizing design data
        if not self.FINE_MODE and not self.UPDATED_TABLE:
            self._update_wing_table()
            self.UPDATED_TABLE = True
        n_row = 12
        fig, ax = plt.subplots(n_row, 1, figsize=(20, n_row*2), sharex=True, facecolor='white')
        ax[0].plot(self.aero.index*0.5*self.span, self.aero['chord'], label='chord', marker=marker)
        ax[1].plot(self.aero.index*0.5*self.span, self.aero['aoa'], label='AoA', marker=marker)
        ax[2].plot(self.aero.index*0.5*self.span, self.aero['local cl']*self.aero['chord']/(self.span/self.aspect_ratio), label='lift', marker=marker)
        ax[2].plot(self.aero.index*0.5*self.span, self.CL*4/np.pi*np.sqrt(1 - self.aero.index**2), label='ellipsoid', marker=marker)
        if self.FINE_MODE:
            ax[3].plot(self.wing.index*0.5*self.span, self.wing['airfoil'], label='Airfoil', marker=marker)
            ax[4].plot(self.wing.index*0.5*self.span, self.wing['re'], label='Re number', marker=marker)
        else:
            ax[3].plot(self.aero.index*0.5*self.span, self.aero['airfoil'], label='Airfoil', marker=marker)
            ax[4].plot(self.aero.index*0.5*self.span, self.aero['re'], label='Re number', marker=marker)
        ax[5].plot(self.wing.index*0.5*self.span, self.wing['diameter'], label='beam diameter', marker=marker)
        ax[6].plot(self.wing.index*0.5*self.span, self.wing['weight'], label='weight', marker=marker)
        ax[7].plot(self.wing.index*0.5*self.span, self.wing['local lift'], label='local lift', marker=marker)
        ax[8].plot(self.wing.index*0.5*self.span, self.wing['M'], label='moment', marker=marker)
        ax[0].set_xlim(0, self.span*0.5)
        ax[0].set_ylim(0, 1.5)
        ax[5].set_ylim(0, 1.5*0.11)
        ax[3].set_yticks(ticks=np.arange(len(self.airfoils)), labels=self.airfoils)
        ax[8].plot(self.wing.index*0.5*self.span, -self.wing['M_zerolift'], label='zero-lift moment', marker=marker)
        ax[9].plot(self.wing.index*0.5*self.span, self.wing['EI'], label='EI', marker=marker)
        ax[10].plot(self.wing.index*0.5*self.span, self.wing['total deflection'], label='deflection', marker=marker)
        ax[10].plot(self.wing.index*0.5*self.span, self.wing['total zerolift deflection'], label='zero-lift deflection', marker=marker)
        ax[11].plot(self.wing.index*0.5*self.span, self.wing['strain'], label='strain', marker=marker)
        ax[11].plot(self.wing.index*0.5*self.span, -self.wing['zerolift strain'], label='zero-lift strain', marker=marker)
        ax[11].axhline(self.allowable_strain, label='constraint', c='black')
        if self.level==0:
            ax[11].axhline(self.allowable_strain*self.strain_ratios[0,:].min(), c='C0', linestyle='dashed', label='deflection control')
            ax[11].axhline(self.allowable_strain*self.strain_ratios[1,:].min(), c='C1', linestyle='dashed', label='zerolift deflection control')
        elif self.level==1:
            for i in range(self.n_div):
                if i == 0:
                    ax[11].plot(self.y_div[i:i+2]*self.span*0.5, np.full(2, self.allowable_strain*self.strain_ratios[0,i]), c='C0', linestyle='dashed', label='deflection control')
                    ax[11].plot(self.y_div[i:i+2]*self.span*0.5, np.full(2, self.allowable_strain*self.strain_ratios[1,i]), c='C1', linestyle='dashed', label='serolift deflection control')
                else:
                    ax[11].plot(self.y_div[i:i+2]*self.span*0.5, np.full(2, self.allowable_strain*self.strain_ratios[0,i]), c='C0', linestyle='dashed')
                    ax[11].plot(self.y_div[i:i+2]*self.span*0.5, np.full(2, self.allowable_strain*self.strain_ratios[1,i]), c='C1', linestyle='dashed')
        for i in range(n_row):
            ax[i].legend()
        if SAVE:
            if len(suffix)>0:
                suffix = '_' + suffix
            plt.savefig(os.path.join(path, 'wing_characteristics'+suffix+'.png'))
    

    def plot_deflection(self, path='./', SAVE=False, suffix=''):
        # for visualizing design data
        if not self.FINE_MODE and not self.UPDATED_TABLE:
            self._update_wing_table()
            self.UPDATED_TABLE = True
        y = self.wing.index*self.span*0.5
        y = np.hstack([-y[::-1], y])
        z = self.wing['total deflection'].values
        z = np.hstack([z[::-1], z])
        z0 = self.wing['total zerolift deflection']
        z0 = np.hstack([z0[::-1], z0])
        fig, ax = plt.subplots(1,1, figsize=(20,10), facecolor='white')
        ax.plot(y, z, linewidth=4, label='flight')
        ax.plot(y, z0, linewidth=4, label='zerolift')
        ax.plot([0,0], [-self.z_wire, 0], c='black', linewidth=4)
        ax.set_aspect('equal')
        ax.grid()
        ax.legend(ncol=2)
        if SAVE:
            if len(suffix)>0:
                suffix = '_' + suffix
            plt.savefig(os.path.join(path, 'wing_deflection'+suffix+'.png'))
    

    def plot_wing3d(self, elev=30, azim=-150, AXIS=False, path='./', SAVE=False, suffix=''):
        # for visualizing design data
        if not self.FINE_MODE and not self.UPDATED_TABLE:
            self._update_wing_table()
            self.UPDATED_TABLE = True
        y = self.wing.index.values*self.span*0.5
        aoa = np.deg2rad(self.wing['aoa'].values)
        z = self.wing['total deflection'].values
        x_l = -self.wing['chord']*0.33
        x_t = self.wing['chord']*0.67
        z_l = -x_l*np.sin(aoa) + z
        z_t = -x_t*np.sin(aoa) + z
        x_l = x_l*np.cos(aoa)
        x_t = x_t*np.cos(aoa)

        X = np.vstack([x_l, x_t]).T
        Y = np.tile(y, [2,1]).T
        Z = np.vstack([z_l, z_t]).T
        X = np.vstack([X[::-1,:], X])
        Y = np.vstack([-Y[::-1,:], Y])
        Z = np.vstack([Z[::-1,:], Z])

        fig = plt.figure('test', figsize=(20,20))
        ax = fig.add_subplot(projection='3d')
        ax.plot_surface(X, Y, Z, facecolor='cyan', edgecolor='gray')
        ax.set_aspect('equal')
        ...
        if SAVE:
            if len(suffix)>0:
                suffix = '_' + suffix
            plt.savefig(os.path.join(path, '3D_wing_shape'+suffix+'.png'))


    def save(self, path='./', DISTRIBUTION=True, PERFORMANCE=True, FIGURE=True):
        if not self.FINE_MODE and not self.UPDATED_TABLE:
            self._update_wing_table()
            self.UPDATED_TABLE = True
        if DISTRIBUTION:
            self.wing.to_csv(os.path.join(path, 'hpa_wing.csv'))
        if PERFORMANCE:
            i_div = np.arange(self.n_div+1).astype(str).astype(object)
            labels = np.hstack(['y'+i_div+' [m]', 'chord '+i_div+' [m]', 'angle of attack '+i_div+' [deg]', 'diameter '+i_div+' [m]', 'airfoil '+i_div,
                                'power [W]', 'velocity [m/s]', 'drag [N]', 'weight [kg]', 'pilot weight [kg]', 'water weight [kg]', 'payload [kg]', 'empty weight [kg]', 'wing weight [kg]', 'wing CFRP weight [kg]', 
                                'span [m]', 'wing area [m^2]', 'mean aerodynamic chord [m]', 'Aspect ratio [-]', 'wing loading [kg/m^2]', 'dihedral angle at root [deg]',
                                'wing tip defrection [m]', 'wing tip deflection at zerolift [m]', 'wire tension [N]', 'horizontal wire fixed point [m]', 'vertical wire fixed point [m]', 
                                'CL', 'CD', 'CDi', 'CD0', 'wing efficiency', 
                                ])
            values = np.hstack([self.y_div*self.span*0.5, self.y_chord, self.y_aoa, self.y_diameter, self.y_airfoil,  
                                self.power, self.v_inf, self.drag, self.weight, self.pilot_weight, self.water_weight, self.payload, self.empty_weight, self.wing_weight.sum()*2, self.beam_weight.sum()*2, 
                                self.span, self.wing_area, self.mac, self.aspect_ratio, self.weight/self.wing_area, self.dihedral_angle_at_root, 
                                self.wing_tip_deflection, self.total_deflection_zerolift[-1], self.wire_tension, self.y_wire*self.span*0.5, self.z_wire, 
                                self.CL, self.CD, self.CDi, self.CD0, self.wing_efficiency, 
                                ])
            path_file = os.path.join(path, 'hpa_performace.csv')
            if os.path.exists(path_file):
                pd.DataFrame(values.reshape(1,-1), columns=labels).to_csv(path_file, index=False, header=False, mode='a')
            else:
                pd.DataFrame(values.reshape(1,-1), columns=labels).to_csv(path_file, index=False)
        if FIGURE:
            self.plot_wing_characteristics(marker='None', path=path, SAVE=True)
            self.plot_deflection(path=path, SAVE=True)
            self.plot_wing3d(elev=30, azim=-150, AXIS=True, path=path, SAVE=True)
...
if __name__ == "__main__":
    ...
    # use baseline
    x = hpa.baseline()
    hpa.evaluate_performance_from_x(x, NORMALIZED=False)

    # or put your design here
    # span = 32 #[m]
    # y_div = np.array([0, 4.6, 8.6, 12.8, 16.0])/(span*0.5)
    # y_chord = np.array([1.05, 1.05, 0.903, 0.7455, 0.462]) #[m]
    # y_aoa = np.array([4.8, 4.8, 4.8, 3.2, 2.0]) #[deg]
    # y_airfoil_name = np.array(['DAE21', 'DAE21', 'DAE31', 'DAE31', 'DAE31'])
    # y_airfoil = np.array([np.argmax(hpa.airfoils==name) for name in y_airfoil_name])
    # y_diameter = np.array([0.101, 0.081, 0.07, 0.04])
    # y_wire = 8.2/(span*0.5)
    # wire_tension = 133.4*hpa.gravity
    # dihedral_angle_at_root = 4.0 #[deg]
    # payload = 0.0 #[kg]
    # n_plys_partial = np.array([8, 8, 8, 6])
    # strain_ratios = np.array([[0.88, 0.88, 0.88, 0.88], [0.66, 0.66, 0.66, 0.66]])
    # hpa.evaluate_performance(span, y_div, y_chord, y_aoa, y_diameter, y_airfoil, y_wire, wire_tension, dihedral_angle_at_root, payload, n_plys_partial, strain_ratios)

    # output
    hpa.save(DISTRIBUTION=True, PERFORMANCE=True, FIGURE=True)
